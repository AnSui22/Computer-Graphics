...
1. C:\LearnOpenGL\src 폴더에 8.robotarm 폴더를 생성하였습니다.
2. 8.robotarm폴더에 1. robotArm폴더를 생성하였습니다.
3. HW3를 다운 받은 후 압축을 풀어 1. robotArm폴더에 HW3의 파일을 모두 복사해서 붙여 넣었습니다.
4. C:\LearnOpenGL\CMakeLists.txt을 다음과 같이 수정하였습니다.
...
5. Cmake GUI로 빌드한 후 RobotArm.cpp에 코드를 추가하였습니다.
**/
...
void myDisplay()
{
	...
	//Draw Base
	//Base는 Y축 방향으로 움직이지 않도록 0.0f로 고정시키고 X, Z축을 따라 이동할 수 있게 한다.
	model = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	model = glm::rotate(model, BaseSpin * 0.02f, glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(model);
	...
	model = glm::rotate(model, ElbowAng * 0.017f, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(model);


	//Draw Wrist
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, WristAng * 0.017f, glm::vec3(0.0f, 0.0f, 1.0f));
	model = glm::rotate(model, WristTwistAng * 0.017f, glm::vec3(0.0f, 1.0f, 0.0f));
	DrawWrist(model);


	//Draw Left Figer Base
	model = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f));
	model = glm::rotate(model, FingerAng1 * 0.017f, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(model);


	//Draw Left Figertip
	glm::mat4 leftfingertip = model;

	leftfingertip = glm::translate(leftfingertip, glm::vec3(0.0f, 0.35f, 0.0f));
	leftfingertip = glm::rotate(leftfingertip, FingerAng2 * 0.017f, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(leftfingertip);


	//Draw Right Figer Base
	model = glm::rotate(model, FingerAng1 * -0.04f, glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(model);


	//Draw Right Figertip
	glm::mat4 rightfingertip = model;

	rightfingertip = glm::translate(rightfingertip, glm::vec3(0.0f, 0.35f, 0.0f));
	rightfingertip = glm::rotate(rightfingertip, FingerAng2 * -0.017f, glm::vec3(0.0f, 0.0f, 1.0f));
